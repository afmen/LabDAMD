...
class RingHashLoadBalancer {
    ...
    maybeProactivelyConnect() {
        ...
        if (!(this.currentState === grpc_js_1.connectivityState.TRANSIENT_FAILURE || this.currentState === grpc_js_1.connectivityState.CONNECTING)) {
            return;
        }
        let firstIdleChild = null;
        for (const leaf of this.leafMap.values()) {
            ...
            if (leafState === grpc_js_1.connectivityState.IDLE && !firstIdleChild) {
                firstIdleChild = leaf;
            }
        }
        if (firstIdleChild) {
    ...
    calculateAndUpdateState() {
        ...
        if (stateCounts[grpc_js_1.connectivityState.READY] > 0) {
            this.updateState(grpc_js_1.connectivityState.READY, new RingHashPicker(this.ring), null);
            // REPORT READY
        }
            ...
            this.updateState(grpc_js_1.connectivityState.TRANSIENT_FAILURE, new UnavailablePicker({ details: errorMessage }), errorMessage);
        }
        else if (stateCounts[grpc_js_1.connectivityState.CONNECTING] > 0) {
            this.updateState(grpc_js_1.connectivityState.CONNECTING, new RingHashPicker(this.ring), null);
        }
            ...
            this.updateState(grpc_js_1.connectivityState.IDLE, new RingHashPicker(this.ring), null);
        }
        else {
            const errorMessage = `ring hash: no connection established. Latest error: ${this.latestErrorMessage}`;
            this.updateState(grpc_js_1.connectivityState.TRANSIENT_FAILURE, new UnavailablePicker({ details: errorMessage }), errorMessage);
    ...
    constructRing(endpointList, config, ringHashSizeCap) {
        this.ring = [];
    ...
    updateAddressList(endpointList, lbConfig, options) {
        ...
        for (const endpoint of endpointList) {
            ...
            if (leafBalancer) {
                ...
                this.leafMap.set(endpoint, new LeafLoadBalancer(endpoint, this.childChannelControlHelper, options));
            }
