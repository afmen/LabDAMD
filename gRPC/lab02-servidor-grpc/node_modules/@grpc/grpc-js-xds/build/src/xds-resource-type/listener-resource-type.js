...
function validateHttpConnectionManager(httpConnectionManager) {
    ...
    switch (httpConnectionManager.route_specifier) {
        case 'rds':
            ...
            break;
        case 'route_config': {
            const routeConfigValidationResult = route_config_resource_type_1.RouteConfigurationResourceType.get().validateResource(httpConnectionManager.route_config);
 ...
 */
function validateTransportSocket(context, transportSocket) {
    const errors = [];
    ...
    if (transportSocket.typed_config.type_url !== resources_1.DOWNSTREAM_TLS_CONTEXT_TYPE_URL) {
        errors.push(`Unexpected transport_socket.typed_config.type_url: ${transportSocket.typed_config.type_url}`);
        return errors;
    }
    const downstreamTlsContext = (0, resources_1.decodeSingleResource)(resources_1.DOWNSTREAM_TLS_CONTEXT_TYPE_URL, transportSocket.typed_config.value);
 ...
 * @returns A list of validation errors, if there are any. An empty list indicates success
 */
function validateFilterChain(context, filterChain) {
    const errors = [];
...
class ListenerResourceType extends xds_resource_type_1.XdsResourceType {
    ...
    validateResource(context, message) {
        ...
        if (message.api_listener?.api_listener) {
            if (message.api_listener.api_listener.type_url === resources_1.HTTP_CONNECTION_MANGER_TYPE_URL) {
                ...
                errors.push(`api_listener.api_listener.type_url != ${resources_1.HTTP_CONNECTION_MANGER_TYPE_URL}`);
            }
        }
